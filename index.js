...
const TREASURY_PROMO_URL = 'https://connect.treasury.id/promotion/suggestion'
...
const promoSubscriptions = new Set() // Subscription untuk broadcast promo ON/OFF
const promoListSubscriptions = new Set() // Subscription untuk broadcast promo LIST
...
// ⚡ CACHE untuk promo data
let cachedPromoData = {
  promoList: null,
  nominalPromo: null,
  lastUpdate: 0
}
let lastPromoStatus = null // Track promo status untuk detect perubahan OFF → ON
let lastPromoStatusChangeTime = 0 // Kapan status terakhir berubah
let lastOnBroadcastTime = 0 // Kapan terakhir broadcast saat ON
let lastOffBroadcastTime = 0 // Kapan terakhir broadcast OFF
const PROMO_CHECK_INTERVAL = 1000 // Tidak dipakai lagi - promo dipanggil dari price update
const ON_BROADCAST_INTERVAL = 60000 // Broadcast setiap 1 menit saat ON
const OFF_SILENCE_DURATION = 300000 // Tidak dipakai lagi
let lastMentionTime = 0 // Track terakhir @mention (1 jam sekali)
const MENTION_INTERVAL = 3600000 // 1 jam dalam ms

// NEW: Promo trigger dari perubahan harga emas
let promoTriggerTimeout = null // Timeout 5 detik setelah harga berubah
let promoCheckInterval = null // Interval cek promo setiap 1 detik (dari detik 5 sampai 57)
let offBroadcastCountThisMinute = 0 // Counter OFF broadcast per menit (max 5)
let lastOffCountResetMinute = 0 // Track kapan reset counter OFF
let lastPromoBroadcastMinute = 0 // Track menit terakhir broadcast promo (untuk 1x/menit)
let isPromoIntervalRunning = false // Flag apakah interval promo sedang jalan
...
// Fetch Treasury Promo List (quiet mode - minimal logging)
async function fetchPromoList(retryCount = 0) {
  try {
    const token = fs.readFileSync(TOKEN_FILE, 'utf-8').trim()

    const res = await fetch(TREASURY_PROMO_URL, {
      method: 'GET',
      headers: {
        'accept': 'application/json',
    ...
    throw new Error(`Promo fetch failed: ${e.message}`)
  }
}

// Fetch Treasury Nominal Promo (quiet mode - minimal logging)
...
// ⚡ BROADCAST FUNCTION - DISABLED (hanya backend, tidak kirim ke WhatsApp)
// Harga emas hanya untuk trigger promo check, tidak dikirim ke subscriber
async function doBroadcast(priceChange, priceData) {
  // TIDAK KIRIM KE WHATSAPP - hanya log di backend
  broadcastCount++
  const buyFormatted = priceData.buy?.toLocaleString('id-ID') || '0'
  const sellFormatted = priceData.sell?.toLocaleString('id-ID') || '0'
  pushLog(`📊 [#${broadcastCount}] Harga: Buy=${buyFormatted}, Sell=${sellFormatted} (backend only, no WA)`)
}

// 🎁 PROMO BROADCAST FUNCTION
// ALUR BARU:
// 1. Harga emas berubah → tunggu 5 detik
// 2. Setelah 5 detik → mulai cek API promo setiap 1 detik
// 3. Cek terus sampai detik ke-57, lalu stop
// 4. Tunggu harga emas berubah lagi → ulangi
//
// BATASAN KIRIM PESAN:
// - ON: Kirim 1x per menit
// - OFF: Kirim 1x per menit, max 5 kali TOTAL, lalu STOP
// - Setelah 5x OFF → tidak kirim lagi sampai ON
// - ON → reset counter OFF
// - TIDAK ADA spam 10x alert
let offStartTime = 0 // Tidak dipakai lagi
...
async function doPromoBroadcast() {
  ...
  // OFF counter TIDAK di-reset per menit
  // Hanya di-reset saat status berubah jadi ON

  try {
    // Fetch nominal data untuk cek ON/OFF
    ...
    const promoNominals = [10000000, 30000000, 40000000, 50000000, 60000000]
    const promoAmounts = [9669000, 29004000, 38672000, 48340000, 58005000]
    const hasPromo = nominalData.data.some(n =>
      n.status === true &&
      (promoNominals.includes(n.default_amount) || promoAmounts.includes(n.promotion_amount))
    )
    ...
    // LOGIKA BROADCAST:
    // - ON: Kirim 1x per menit, reset counter OFF
    // - OFF→ON: Kirim LANGSUNG (bypass throttle), dengan mention semua anggota
    // - OFF: Kirim 1x per menit, max 5 kali total, lalu STOP sampai ON lagi

    let shouldSendMessage = false

    if (currentStatus === 'ON') {
      // ON: Reset counter OFF
      if (offBroadcastCountThisMinute > 0) {
        pushLog(`🎁 Status ON - Reset OFF counter (was ${offBroadcastCountThisMinute})`)
        offBroadcastCountThisMinute = 0
      }

      // OFF→ON: Selalu kirim langsung (bypass per-menit throttle)
      if (isOffToOn) {
        shouldSendMessage = true
        lastPromoBroadcastMinute = currentMinute
      } else if (currentMinute !== lastPromoBroadcastMinute || isFirstCheck) {
        // ON biasa: Hanya kirim 1x per menit
        shouldSendMessage = true
        lastPromoBroadcastMinute = currentMinute
      }
    } else {
      // OFF: Sudah 5x total? STOP sampai ON lagi
      if (offBroadcastCountThisMinute >= 5) {
        // Tidak kirim, tidak log (silent)
        return
      }
      ...
      if (currentMinute !== lastPromoBroadcastMinute || isFirstCheck) {
        shouldSendMessage = true
        offBroadcastCountThisMinute++
        lastPromoBroadcastMinute = currentMinute
      }
    }

    // Update lastPromoStatus
    lastPromoStatus = currentStatus

    if (!shouldSendMessage) {
      // Sudah kirim di menit ini, skip (silent - tidak perlu log)
      return
    }
    ...
    pushLog(`🎁 Broadcasting: ${currentStatus} to ${promoSubscriptions.size} subscribers ${isOffToOn ? '(OFF→ON MENTION!)' : currentStatus === 'OFF' ? `(OFF ${offBroadcastCountThisMinute}/5)` : ''}`)

    // Kirim ke semua subscribers
    for (const chatId of promoSubscriptions) {
      try {
        // OFF→ON: mention semua anggota (hanya untuk grup)
        if (isOffToOn && chatId.endsWith('@g.us')) {
          const gm = await sock.groupMetadata(chatId)
          const emasmentions = gm.participants.map(p => p.id)
          await sock.sendMessage(chatId, { text: message, mentions: emasmentions })
        } else {
    ...
    // Update timestamp
    if (currentStatus === 'ON') {
      lastOnBroadcastTime = now
    } else {
      lastOffBroadcastTime = now
    }
...
// doPromoBroadcast tidak pakai setInterval lagi - dipanggil dari checkPriceUpdate saat harga berubah
  ...
  sock.ev.on('messages.upsert', async (ev) => {
    ...
    for (const msg of ev.messages) {
      try {
        ...
        // DISABLED: Command aktif (gunakan cekon)
        // if (/\baktif\b/.test(text)) { ... }

        // DISABLED: Command nonaktif (gunakan cekonmati)
        // if (/\bnonaktif\b/.test(text)) { ... }

        // DISABLED: Command cekpromo (gunakan cekon untuk subscribe)
        // if (/\bcekpromo\b/.test(text)) { ... }

        // DISABLED: Command telpon
        // if (/\btelpon\b/.test(text)) { ... }

        // ============================================
        // COMMAND AKTIF: cekon & cekonmati
        // ============================================

        // Command: cekon (subscribe broadcast promo ON/OFF saja)
        // Harga emas hanya jalan di backend untuk trigger, tidak dikirim ke WhatsApp
        if (/\bcekon\b/.test(text)) {
          promoSubscriptions.add(sendTarget)
          subscriptions.add(sendTarget) // Tetap add untuk trigger price check di backend
          pushLog(`➕ ON/OFF sub: ${sendTarget.substring(0, 15)} (total: ${promoSubscriptions.size})`)
        ...
        // Command: cekonmati (unsubscribe broadcast promo ON/OFF)
        if (/\bcekonmati\b/.test(text)) {
          promoSubscriptions.delete(sendTarget)
          subscriptions.delete(sendTarget) // Juga hapus dari price check trigger
          pushLog(`➖ ON/OFF unsub: ${sendTarget.substring(0, 15)} (total: ${promoSubscriptions.size})`)
          await sock.sendMessage(sendTarget, { text: '🎉 Berhenti' }, { quoted: msg })
          continue
        }

        // DISABLED: Command ceklistpromoaktif
        // if (/\bceklistpromoaktif\b/.test(text)) { ... }

        // DISABLED: Command ceklistpromononaktif
        // if (/\bceklistpromononaktif\b/.test(text)) { ... }

        // DISABLED: Command emas (manual check) - tidak ada command lagi selain cekon/cekonmati
        // if (/\bemas\b/.test(text)) { ... }

        // Skip semua text lain yang bukan cekon/cekonmati
        // (tidak ada command lain yang aktif)
